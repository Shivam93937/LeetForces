...
const Resetpwd:React.FC<ResetpwdProps> = () => {
    ...
    const handleReset=async (e:React.FormEvent<HTMLFormElement>)=>{
        ...
        const success = await sendPasswordResetEmail(email);
          if (success) {
            toast.success("Please check your e-mail for reset link",{position:"top-center",autoClose:3000, theme:"dark"});
          }
    }

    useEffect(()=>{
        if(error){
            toast.error("Please register this e-mail",{position:"top-center",autoClose:3000, theme:"dark"});
        }
    })
