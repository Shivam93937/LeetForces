...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
	...
	const handleLike=async () =>{
		if(!user){
			toast.error("You most be logged-in to like a problem",{position:"top-center", theme:"dark"});
			return;
		}
		if(updating) return;
		setUpdating(true);
		// if already liked, if already disliked or neither of them

		await runTransaction(firestore, async(transaction)=>{
			const {userRef,problemRef,userDoc,problemDoc}= await returnUserDataAndProblemData(transaction);
	...
	const handleDislike = async()=>{
		if(!user){
			toast.error("You most be logged-in to dislike a problem",{position:"top-center", theme:"dark"});
			return;
		}
		if(updating) return;
		setUpdating(true);
		await runTransaction(firestore, async(transaction)=>{
			const {userRef,problemRef,userDoc,problemDoc} = await returnUserDataAndProblemData(transaction);
			if(userDoc.exists() && problemDoc.exists){
				//if already disliked, if already liked, neither of them 
				if(disliked){
					transaction.update(userRef,{
						dislikedProblems:userDoc.data().dislikedProblems.filter((id:string)=> id!==problem.id)
					});
	...
	const handleStar=async ()=>{
		if(!user){
			toast.error("You most be logged-in to star a problem",{position:"top-center", theme:"dark"});
			return;
		}
		if(updating) return;
		setUpdating(true);
		...
		const getCurrentProblem = async ()=>{
			...
			if(docSnap.exists()){
				...
				setProblemDifficultyClass(
					problem.difficulty === "Easy"?"bg-olive text-olive" : problem.difficulty === "Medium" ? "bg-dark-yellow text-dark-yellow": "bg-dark-oink text-dark-pink"
				)
			}
			setLoading(false);
		};
